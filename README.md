...
## 教程视频
[【工具】QQ音乐解密mflac教程](https://www.bilibili.com/video/BV1aRPCzuELn/?share_source=copy_web&vd_source=f447c8bfbd4e18e776bf5087c3a430e2)
...
- 程序会在**当前运行目录**自动创建 `plugins/plugins.json`，用于保存输入输出路径与运行选项
...
- 解密后的音频文件仅限个人欣赏
- 请尊重版权，支持正版音乐
- 若输出目录包含中文，程序会把临时文件写入同盘符的 `_qqmusic_tmp`，再移动到目标路径
- 如需反馈问题，请优先附上当前运行目录下 `_log/<YYYY-M-D>/` 的对应日志文件
