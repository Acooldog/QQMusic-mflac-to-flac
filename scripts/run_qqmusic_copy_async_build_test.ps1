...
$scriptPath = Join-Path $scriptDir 'qqmusic_copy_async_build_test.py'

if (-not (Test-Path $python)) {
    throw "未找到虚拟环境 Python: $python"
}
if (-not (Test-Path $scriptPath)) {
    throw "未找到 copy_async_build 测试脚本: $scriptPath"
}

Write-Host '启动 QQMusic copy_async_build 实验...' -ForegroundColor Cyan
