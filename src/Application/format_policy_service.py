...
class FormatPolicyService:
    ...
    def supported_sources(self) -> List[str]:
        return sorted(self.DEFAULT_RULES.keys())
    ...
    def target_format(self, src_ext: str, rules: Dict[str, str]) -> str:
        ...
        if target and target in self.FORMAT_WHITELIST:
            return target
        return self.default_format(key)
    ...
    def normalize_rules(self, rules: Dict[str, str]) -> Dict[str, str]:
        ...
        for src_ext in self.DEFAULT_RULES:
            ...
            if requested not in self.FORMAT_WHITELIST:
                logger.warning(
                    "Invalid format rule, fallback to default: %s -> %s (supported: %s)",
                    src_ext,
                    requested,
    ...
    def normalize_overrides(self, overrides: Dict[str, str]) -> Dict[str, str]:
        source = overrides if isinstance(overrides, dict) else {}
        normalized: Dict[str, str] = {}
        for src_ext, value in source.items():
            key = self.normalize_source_ext(src_ext)
            if key not in self.DEFAULT_RULES:
                continue
